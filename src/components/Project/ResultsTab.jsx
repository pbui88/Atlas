...
export default function ResultsTab({ project }) {
  ...
  // Mount or reposition Street View panorama when selection changes
  useEffect(() => {
    if (!selected || !svContainerRef.current || !window.google?.maps) return
  ...
  // Tear down panorama when deselected
  useEffect(() => {
    if (!selected) svPanoRef.current = null
  }, [selected])
      ...
      <div className="flex-1 flex flex-col bg-slate-950">
        {selected ? (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-3">
            ...
            <p className="text-sm text-slate-500">Select a property to view Street View</p>
          </div>
        )}
      </div>
    </div>
