                ...
                <tr key={user.id} className="hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => toggleRole(user)} className="text-xs text-slate-500 hover:text-slate-300 transition">
                        {user.role === 'admin' ? 'Demote' : 'Promote'}
                      </button>
                      <button onClick={() => toggleActive(user)} className="text-xs text-slate-500 hover:text-yellow-400 transition">
                        {user.is_active ? 'Suspend' : 'Activate'}
                      </button>
                      <button onClick={() => deleteUser(user)} className="text-xs text-slate-500 hover:text-red-400 transition">
                        Delete
                      </button>
                    </div>
                  </td>
            ...
            <div className="divide-y divide-slate-800">
              {usage.byService.map(row => (
                <div key={row.service} className="flex items-center justify-between py-3 text-sm">
                  <span className="text-slate-400 capitalize">{row.service.replace(/_/g, ' ')}</span>
                  <div className="text-right">
                    <span className="text-slate-200 font-medium">{row.total_count.toLocaleString()} calls</span>
                    {row.total_cost != null && (
                      <span className="text-slate-500 ml-3">${(+row.total_cost).toFixed(2)}</span>
                    )}
                  </div>
                </div>
              ))}
